...
[`guardroom/configs/shop.json`](../guardroom/configs/shop.json) 的 monitors 項目，
...
預設 JSONL 路徑為目前工作目錄的 `tmp/monitor.jsonl`，可用 `NIGHTWATCH_MONITOR_LOG` 覆寫；Example Shop 將它掛載到 host `.run/monitor/monitor.jsonl`，時間改為 UTC RFC3339；
...
`ConsoleLogSink` 將原始事件投影為 `guardroom/frontend/schema-draft/log.schema.json` 格式，
...
先啟動 `./guardroom/deploy/restart.sh`，應用程式再建立 sink：
...
Shop-web 預設透過 Compose 共用的 `.run/monitor/monitor.jsonl` 讓 Guard Room 讀取，
