...
此命令使用 Docker Compose，預設 port 9999。若要在 host 開發，先以 `./guardroom/deploy/restart.sh --close` 停止容器，再安裝既有鎖定依賴並啟動單一 worker：

```sh
uv sync --project guardroom/backend --locked --offline
guardroom/backend/.venv/bin/python -m uvicorn main:app --app-dir guardroom/backend --host 127.0.0.1 --port 9999
```
...
前端路由、相容策略與驗收記錄見 [調查對話 spec](../frontend/specs/investigation-chat-report.md)。
