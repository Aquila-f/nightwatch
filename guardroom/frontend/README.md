...
```sh
bash guardroom/frontend/restart.sh
```
...
```sh
NIGHTWATCH_CONTROL_URL=http://127.0.0.1:8001 bash guardroom/frontend/restart.sh
```

PID 與日誌位於 `guardroom/frontend/.codex/run/console-<port>.pid`、`console-<port>.log`。腳本檢查首頁與調查 API；上游檢查失敗會回傳非零狀態並保留 Console，方便查看錯誤。Shop 與 Guard Room 需另外啟動。
...
```sh
python3 guardroom/frontend/serve.py --port 4174 --mock
```
...
顯示篩選與 log 容量測試：`node --experimental-default-type=module --test guardroom/frontend/tests/live-performance.test.mjs`。
