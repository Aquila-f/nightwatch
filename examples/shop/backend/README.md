...
`examples/shop/backend/` contains the FastAPI public gateway and the order service for 日日選物. The gateway keeps the existing public paths and validation schemas while forwarding product, cart, and checkout work to separate service owners. Full public fields and examples are in [../docs/API.md](../docs/API.md).
...
From `examples/shop/backend/`, the repository environment can be installed with the locked dependencies:
